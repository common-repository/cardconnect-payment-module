<?php
	...
	/**
	 * This class enables the gateway to integrate with the old/deprecated Subscriptions extension
	 * Deprecated - Intended to support Subscriptions 1.x !
	 *
	 * @since 0.6.0
	 */
	class CardConnectPaymentGatewayAddonsDeprecated extends CardConnectPaymentGateway {
		...
		public function __construct() {
			...
			if (class_exists('WC_Subscriptions_Order')) {
				...
				add_filter('woocommerce_my_subscriptions_recurring_payment_method', array(
					$this,
		 ...
		 * Process the payment
		 *
		 * If order contains subscriptions, use this class's process_subscription. If not, use the main class's
		 ...
		 *
		 * Saves the card, if needed, and activates the subscription. This is called when the subscription is first
		 * purchased
		 *
		 * @param int $order_id
		 *
		...
		public function maybe_render_subscription_payment_method($payment_method_to_display, $subscription_details, WC_Order $order) {
			...
			if ($this->id !== $order->recurring_payment_method || !$order->customer_user) return $payment_method_to_display;
