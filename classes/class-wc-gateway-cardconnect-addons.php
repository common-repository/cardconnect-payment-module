	...
	class CardConnectPaymentGatewayAddons extends CardConnectPaymentGateway {
		...
		public function __construct() {
			...
			if (class_exists('WC_Subscriptions_Order')) {
				...
				// Allow STORE MANAGERS/ADMINS to manually change/set CardPointe as the payment method on a subscription (via wp-admin)
		...
		public function generate_cardconnect_request($order, $amountToCharge = NULL, $checkoutFormData = NULL) {
			...
			$amountToCharge = $this->get_order_total_formatted($order);
			...
			$user_fields = parent::get_user_defined_form_fields($order, $order_meta, true);
			if (!empty($user_fields)) {
				$request['userfields'] = $user_fields;
			}
			if (isset($this->is_recurring) && $this->is_recurring) {
				$request['woo_type'] = 'sr';
			}
			...
			if ($saved_card_id) {
				...
				$request['cof'] = 'M';
				$request['cofscheduled'] = $amountToCharge > 0 ? 'Y' : 'N';
			...
			elseif ($profile_id) {
				...
				$profile_request = array(
					...
					'cvv2'       => $cvv2,
					'expiry'     => $expiry,
					'name'       => $card_name ? $card_name : trim($order->get_billing_first_name() . ' ' . $order->get_billing_last_name()),
					'address'    => $order->get_billing_address_1(),
					'city'       => $order->get_billing_city(),
					'region'     => $order->get_billing_state(),
					'country'    => $order->get_billing_country(),
		...
		public function handle_cardconnect_response($payment_response, $order, $checkoutFormData) {
			...
			if ((isset($payment_response['result'])) && ($payment_response['result'] != 'success')) {
				...
				$order->set_status('failed', __('Payment Failed', 'cardconnect-payment-gateway'));
				$order->save();
		...
		public function scheduled_subscription_payment($amount_to_charge, $order) {
			...
			// set flag to ignore for new PEST file security:
		...
		public function process_subscription($order_id) {
			...
			// -----------------------------------------------------------------------
			$payment_response = $this->process_subscription_payment($order, $this->get_order_total_formatted($order), $request, true);
		...
		public function process_subscription_payment($order = '', $amountToCharge = 0, $additionalRequest = NULL, $showNotices = false) {
			...
			if (!$additionalRequest) {
				...
				$profileid = get_post_meta($order->get_id(), '_wc_cardconnect_profileid', true);
				$acctid = get_post_meta($order->get_id(), '_wc_cardconnect_acctid', true);
				...
				if ($profileid == '') {
					...
					$profileid = get_user_meta($order->get_user_id(), 'wc_cardconnect_profile_id', true);
			...
			if ((!$response) || ('' === $response)) {
				...
				update_post_meta($order->get_id(), '_wc_cardconnect_last4', substr(trim($response['account']), -4));
				update_post_meta($order->get_id(), '_wc_cardconnect_profileid', $response['profileid']);
				update_post_meta($order->get_id(), '_wc_cardconnect_acctid', $response['acctid']);
				...
				foreach (wcs_get_subscriptions_for_order($order->get_id()) as $subscription) {
					...
					update_post_meta($subscription->get_id(), '_wc_cardconnect_last4', substr(trim($response['account']), -4));
					update_post_meta($subscription->get_id(), '_wc_cardconnect_profileid', $response['profileid']);
					update_post_meta($subscription->get_id(), '_wc_cardconnect_acctid', $response['acctid']);
				}
				...
				$order->add_order_note(sprintf(__('CardPointe payment processed (ID: %s, Authcode: %s, Amount: %s)', 'woocommerce'), $response['retref'], $response['authcode'], get_woocommerce_currency_symbol() . ' ' . $response['amount']));
				...
				// record version for troubleshooting
				update_post_meta($order->get_id(), '_cardconnect_order_version', WC_CARDCONNECT_VER);
		 ...
		 * @access public
		 *
		 * @param int $resubscribe_order The order created for the customer to resubscribe to the old expired/cancelled
		 *                               subscription
		 *
		 * @return void
		 */
		 ...
		 * @param WC_Order $renewal_order The order which recorded the successful payment (to make up for the
		 *                                       failed automatic payment).
		 */
		public function update_failing_payment_method($subscription, $renewal_order) {
			$renewal_order = wc_get_order($renewal_order);

			$last4 = get_post_meta($renewal_order->get_id(), '_wc_cardconnect_last4', true);
			$profileid = get_post_meta($renewal_order->get_id(), '_wc_cardconnect_profileid', true);
			$acctid = get_post_meta($renewal_order->get_id(), '_wc_cardconnect_acctid', true);


			// Also save meta info on the Subscription that this renewal order is tied to
			$subscription_id = get_post_meta($renewal_order->get_id(), '_subscription_renewal', true);
		...
		public function maybe_render_subscription_payment_method($payment_method_to_display, $subscription) {
			...
			if ($this->id !== $payment_gateway->id) {
				...
				$last4 = get_post_meta($subscription->get_id(), '_wc_cardconnect_last4', true);
				if (trim($last4) != '') {
		...
		public function add_subscription_payment_meta($payment_meta, $subscription) {
			...
			$profileid = get_post_meta($subscription->get_id(), '_wc_cardconnect_profileid', true);
			$acctid = get_post_meta($subscription->get_id(), '_wc_cardconnect_acctid', true);
			...
			$payment_meta[$this->id] = array(
				'post_meta' => array(
					'_wc_cardconnect_profileid' => array(
						'value' => $profileid,
						'label' => 'CardPointe Profile ID',
					),
					'_wc_cardconnect_acctid'    => array(
						'value' => $acctid,
						'label' => 'CardPointe Account ID',
					),
		...
		function validate_subscription_payment_meta($payment_method_id, $payment_meta) {
			...
			if ($this->id === $payment_method_id) {
				...
				if (!isset($payment_meta['post_meta']['_wc_cardconnect_profileid']['value']) || empty($payment_meta['post_meta']['_wc_cardconnect_profileid']['value'])) {
					throw new Exception('New payment method CardPointe is missing a Profile ID');
				}
				...
				if (!isset($payment_meta['post_meta']['_wc_cardconnect_acctid']['value']) || empty($payment_meta['post_meta']['_wc_cardconnect_acctid']['value'])) {
					throw new Exception('New payment method CardPointe is missing an Account ID');
				}
				...
				if (isset($response[0]) && isset($response[0]['token'])) {
					...
					throw new Exception('New Payment Method\'s CardPointe ProfileID and Account ID are not valid!  Please try again');
				}
		...
		public function process_pre_order($order_id) {
			...
			if (WC_Pre_Orders_Order::order_requires_payment_tokenization($order_id)) {
				...
				if ($prepare_payment_response && !isset($prepare_payment_response['result'])) {
					...
					$order = wc_get_order($order);
					update_post_meta($order->get_id(), '_wc_cardconnect_last4', substr(trim($prepare_payment_response['account']), -4));
					update_post_meta($order->get_id(), '_wc_cardconnect_profileid', $prepare_payment_response['profileid']);
					update_post_meta($order->get_id(), '_wc_cardconnect_acctid', $prepare_payment_response['acctid']);

					// -----------------------------------------------------------------------

					$order->add_order_note(sprintf(__('CardPointe pre-order payment pre-approved (ID: %s, Authcode: %s)', 'woocommerce'), $prepare_payment_response['retref'], $prepare_payment_response['authcode']));
		...
		public function process_pre_order_release_payment($order) {
			...
			if (isset($payment_response['retref']) && isset($payment_response['authcode'])) {
				...
				$order->add_order_note(sprintf(__('CardPointe pre-order payment processed (ID: %s, Authcode: %s, Amount: %s)', 'woocommerce'), $payment_response['retref'], $payment_response['authcode'], get_woocommerce_currency_symbol() . ' ' . $payment_response['amount']));
